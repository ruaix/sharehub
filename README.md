...
> 当前处于早期开发阶段。核心认证、项目、租期和订单链路已经接入后端，报表、通知和部分高级管理能力仍在完善。
...
前端默认请求 `http://localhost:8787/api`，可通过构建环境变量 `NEXT_PUBLIC_API_URL` 修改，因此前后端默认分别使用 3000 和 8787 端口。
...
- 项目编辑、租期续费表单和订阅链接轮换界面
- 仪表盘统计、收入报表和导出
- 探针状态自动同步与异常通知
- 管理端用户禁用、角色调整与更细的数据范围管理
